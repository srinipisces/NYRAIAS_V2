...
const LoginPage = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        const fullPath = `${window.location.origin}/activatedcarbon/${accountid}${nextPath}`;
  ...
  const handleLogin = async () => {
    try {
      ...
      // Step 1: login and get route from backend
      ...
      const { route } = loginRes.data;

      // Step 2: redirect to that route
      window.location.href = `${window.location.origin}${route.startsWith('/') ? '' : '/'}${route}`;
    } catch (err) {
