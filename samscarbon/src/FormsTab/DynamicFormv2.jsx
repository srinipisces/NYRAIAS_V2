...
const DynamicFormv2 = forwardRef(({ fields, defaultValues = {}, onSubmit, onValueChange }, ref) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const loadOptions = async () => {
    const fetchers = fields.map(async (field) => {
      const { name, optionsFunctionName } = field;
      if (optionsFunctionName) {
        setLoadingMap((prev) => ({ ...prev, [name]: true }));
        const opts = await fetchOptionsFromUrl(optionsFunctionName);
      ...
      >
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gridColumn: '1 / -1' }}>
          <Button
            ...
            variant="contained"
            size="small"
            sx={{ height: 36, minWidth: 120, fontSize: 14 }}
          >
            Submit
          </Button>
        </Box>
