...
function pie_default() {
  var value = identity_default, sortValues = descending_default, sort3 = null, startAngle = constant_default(0), endAngle = constant_default(tau), padAngle = constant_default(0);
  function pie(data) {
    var i, n = (data = array_default(data)).length, j, k2, sum4 = 0, index3 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(tau, Math.max(-tau, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
    for (i = 0; i < n; ++i) {
...
function ticker(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start, stop, count3) {
    ...
    if (i === tickIntervals.length) return year2.every(tickStep2(start / durationYear, stop / durationYear, count3));
...
function ticker2(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start, stop, count3) {
    ...
    if (i === tickIntervals.length) return year2.every(tickStep(start / durationYear2, stop / durationYear2, count3));
    if (i === 0) return millisecond2.every(Math.max(tickStep(start, stop, count3), 1));
    const [t, step] = tickIntervals[target / tickIntervals[i - 1][2] < tickIntervals[i][2] / target ? i - 1 : i];
    return t.every(step);
  }
  return [ticks2, tickInterval];
...
function time() {
  return initRange.apply(calendar(timeTicks2, timeTickInterval2, timeYear, timeMonth, timeSunday, timeDay, timeHour, timeMinute, second2, timeFormat).domain([new Date(2e3, 0, 1), new Date(2e3, 0, 2)]), arguments);
}

// node_modules/d3-scale/src/utcTime.js
function utcTime() {
  return initRange.apply(calendar(utcTicks2, utcTickInterval2, utcYear2, utcMonth2, utcSunday2, utcDay2, utcHour2, utcMinute2, second2, utcFormat).domain([Date.UTC(2e3, 0, 1), Date.UTC(2e3, 0, 2)]), arguments);
}
...
//# sourceMappingURL=chunk-ITZZVA67.js.map
