...
const reportConfigs = [
  {
    id: "operations_loaded",
    name: "Loaded Bags",
    description:
      "Bags Loaded operations (Screening/Crushing/Blending/De-Dusting/De-Magnetize) wise.",
    endpoint: { method: "GET", path: "/api/reports_postactivation/operations_loaded" },
    csv:      { method: "GET", path: "/api/reports_postactivation/operations_loaded.csv" },
    pageSize: 50,
    fields: [
      {
        name: "Operation",
    ...
    // Build query params/body the backend expects
    buildParams: ({ filters, page, pageSize, sort, dir }) => {
      const params = {
        page,
        pageSize,
        sort: sort || "loaded_time",
        dir:  dir  || "desc",
      };
    ...
    // Parse response to table data + total
    readResponse: (res) => {
      const { rows = [], total = 0 } = res.data || {};
      return { rows, total };
    },
    // Chart appears when BOTH dates present and range <= 30 days
    /* shouldShowChart: (filters) => {
      const { from, to } = filters || {};
      return isValidYMD(from || "") && isValidYMD(to || "") && daysBetween(from, to) <= 30;
    }, */
    // Build simple per-day count series
    buildChart: (rows) => {
      const counts = new Map();
      for (const r of rows) {
        const day = (r.loaded_time || "").slice(0, 10);
        if (!day) continue;
