...
export default function Re_Process({tabName}) {
  // Tabs that should HIDE grade settings
    const HIDE_GRADE_SETTINGS_TABS = new Set(['De-Dusting','De-Magnetize','Blending']);
    const showGradeSettings = !HIDE_GRADE_SETTINGS_TABS.has(tabName);
    const settingsHidden = !showGradeSettings;
  ...
  const [scanCap, setScanCap] = useState({ ok: false, reason: "" });

    useEffect(() => {
      (async () => {
        if (!isSecureContextOK()) {
          setScanCap({ ok: false, reason: "Camera requires HTTPS (or localhost)" });
          return;
        }
        const camera = await hasVideoInput();
        if (!camera) {
          setScanCap({ ok: false, reason: "No camera detected" });
          return;
        }
        // We’ll run ZXing or BarcodeDetector; either needs camera, which we have.
        setScanCap({ ok: true, reason: "" });
      })();
    }, []);
    ...
    try {
      ...
      if (data?.busy) {
        ...
        setServerOutBags(
            (data?.out_bags ?? []).map(x => ({
                bag_no: x.bag_no,
                bag_weight: x.bag_weight ?? x.weight,
                grade: x.grade,                           // ← use backend grade
                bag_no_created_dttm: x.bag_no_created_dttm ?? x.created_dttm ?? null,
            }))
        );
  ...
  // Unique grades from the currently loaded bags - for De-Dusting,De-Magnetize and Blending
    const getBagGrade = (bag = {}) =>
        bag.grade ??
        bag.output_grade ??
        bag.kiln_grade ??
        bag.grade_name ??
    ...
    const arr = loaderBags
        .map((b) => getBagGrade(b))
        .filter(Boolean);
    return Array.from(new Set(arr)); // dedupe
    }, [loaderBags]);

    // When grade settings are hidden, drive selectedGrades from loaded bags (deduped)
    useEffect(() => {
    if (!showGradeSettings) {
        setSelectedGrades(derivedGrades);
        // If you persist live grades, you can also do:
        // saveLiveGrades(derivedGrades, "Auto from loaded bags (hidden tabs)").catch(()=>{});
    }
    }, [showGradeSettings, derivedGrades]);
...
const startScreening = async () => {
  const gradesToUse = showGradeSettings ? selectedGrades : derivedGrades;

  if (!loaderBags.length || gradesToUse.length === 0) return;
  try {
  ...
  setStarting(true);
  try {
    const bagsPayload = loaderBags.map(({ bag_no, weight ,grade }) => ({ bag_no, weight ,grade}));
...
// Qr code------------
  ...
  function extractBagNoFromScan(raw = "") {
    ...
    // Try URL first: ?bag_no=... or path segment
    try {
      const u = new URL(txt);
      for (const key of ["bag_no", "bag", "id"]) {
    ...
    // Try loose query string (e.g. "bag_no=KOA_...&x=1")
    try {
  ...
  const badScanRef = useRef({ text: "", t: 0 });

  const handleQrDetected = (raw) => {
      // keep whatever tracking you already have:
      if (typeof setLastScannedQR === "function") setLastScannedQR(raw);

      const candidate = extractBagNoFromScan(raw);
      const bn = normalizeBagNo(candidate);

      // helper: alert once per repeated bad frame
      const alertOnce = (msg) => {
        const now = Date.now();
      ...
      // junk / empty
      if (!bn) {
        alertOnce("Not a valid QR.");
        return;
      }

      // already in loader? do nothing
      if (loaderBags.some(b => normalizeBagNo(b.bag_no) === bn)) return;

      // must exist in Available
      const idx = available.findIndex(b => normalizeBagNo(b.bag_no) === bn);
      if (idx === -1) {
        alertOnce(`Not a valid QR for this station (bag not in Available): ${bn}`);
        return;
      }

      // cap = 4 bags (silent if full as per your preference)
      if (loaderBags.length >= 4) return;

      const bag = available[idx];

      // use your existing helper if present
      if (typeof addBagToLoader === "function") {
        addBagToLoader(bag);
      } else {
        // fallback: add to loader & remove from available
        setLoaderBags(prev => [...prev, bag]);
        setAvailable(prev => prev.filter(x => normalizeBagNo(x.bag_no) !== bn));
      }

      // success: no alert/snackbar
              ...
              <Stack direction="row" spacing={2} alignItems="center">
                <Stack direction="row" spacing={1} alignItems="center">
                  <Chip
                    ...
                    size="small"
                  />
                  <Tooltip title={busy ? "Scanner disabled while machine is busy" : "Scan a QR code"}>
                    <span>
                      <IconButton
                        size="small"
                        onClick={() => setScanOpen(true)}
                        disabled={busy || statusLoading || !scanCap.ok}
                      >
                  ...
                  {busy ? (
                    <>
                      ...
                      <Box sx={{ overflowY: "auto", pr: 1,maxHeight:{ xs: "100%", sm: 200 } }}>
                        {showGradeSettings && selectedGrades.length === 0 && (
                          <Alert severity="warning" sx={{ mb: 1 }}>Select at least one grade.</Alert>
                        )}
                        {(showGradeSettings ? selectedGrades : derivedGrades).map((g) => {
