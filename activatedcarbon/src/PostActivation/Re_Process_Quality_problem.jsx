...
export default function Re_Process_Quality() {
  ...
  // ---------- Panels (bags + data-entry) ----------
  const renderBagsPanel = () => (
    <Paper
      elevation={3}
      sx={{ p: 1.25, borderRadius: 2, bgcolor: "background.paper", width: "100%", maxWidth: "100%", overflowX: "hidden" }}
    >
      <TextField
    ...
    <Box sx={{ width: "100%", maxWidth: 1200, mx: "auto", p: { xs: 1, sm: 1.5 } }}>
      ...
      >
              ...
              <Stack direction="row" justifyContent="center" spacing={1} alignItems="center">
                ...
                <Chip
                  ...
                  label={
                    busy
                      ? "Fetching data…"
                      : (alias ? `Alias: ${alias} • Params: ${metricsDef.length}` : "No alias • CTC only")
                  }
                />
              </Stack>

              {/* Bags panel stays in the header ONLY on small screens */}
              {!isMdUp && renderBagsPanel()}
            </Stack>
          }
        />
        ...
        <CardContent sx={{ pt: 2, overflowX: "hidden" }}>
          ...
          {/* On md+ split into left (bags) and right (data-entry). On xs/sm show data-entry only. */}
          {isMdUp ? (
