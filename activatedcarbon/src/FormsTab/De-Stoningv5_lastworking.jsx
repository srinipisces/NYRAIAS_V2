    ...
    >
      ...
      >
        ...
        >
            <Box display="flex" alignItems="center" gap={2} mb={2}>
                ...
                <Button variant="contained" color="success" sx={{ minWidth: 60, height: 30, fontSize: '0.75rem' }}>Load</Button>
            </Box>

          <Box flex={1} overflow="auto">
            {loaderItems.map((item) => (
              <Box key={item} display="flex" alignItems="center" mb={1} bgcolor="#ccc" p={0.5} borderRadius={1}>
                <Button size="small" variant="contained" onClick={() => handleUnload(item)} sx={{ minWidth: 24 }}>{"<"}</Button>
      ...
      <Box mt={2} display="flex" flexDirection="column" alignItems="flex-start">
        <Box display="flex" justifyContent="center" alignItems="center" gap={2} mb={2}>
          <img src="/bag.png" alt="Bag" style={{ height: '40px' }} />
          <Box>
            <Typography fontSize="0.8rem">Enter Weight</Typography>
            <input type="number" placeholder="Weight Out" style={{ width: '100px', padding: '4px' }} />
          </Box>
        </Box>
        <Box display="flex" justifyContent="center" gap={2}>
          <Button variant="contained" color="primary">Screening</Button>
          <Button variant="contained" color="secondary">InStock</Button>
        </Box>
      </Box>
    </Box>
  );
