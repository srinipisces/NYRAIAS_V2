...
export default function LoadBagsMenu({
  ...
  const [wantScanner, setWantScanner] = React.useState(false);
  // when popover finishes closing (open === false), open the scanner
  React.useEffect(() => {
    if (!open && wantScanner) {
      setScannerOpen(true);
      ...
      >
         <Button
          ...
          /* onClick={() => {
            ...
            setWantScanner(true);
            //setTimeout(() => setScannerOpen(true), 0); // then open scanner on next tick
          }} */
          // onClick={() => onOpenScanner?.(handleScanDetected)}
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();           // <-- critical
            onOpenScanner?.(handleScanDetected);
          }}
