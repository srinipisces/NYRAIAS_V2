...
export default function DeStoningTable({ searchText }) {
  const [tableData, setTableData] = useState({ columns: [], rows: [] });

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/kiln/de-stoning`, {
          withCredentials: true,
        });
        setTableData(res.data); // { columns: [...], rows: [...] }
      } catch (err) {
        console.error('Error loading de-stoning:', err.message);
  ...
  const handlePopupSave = async (row, field, newValue) => {
    ...
    await axios.post(`${API_URL}/api/kiln/destoning-update-cell`, payload, {
      withCredentials: true,
    });
  ...
  return (
    <Box sx={{ padding: 2 }}>
      <DynamicTable
        ...
        rows={filteredRows}
        primaryKey="bag_no"
        editablePopupFields={['destoning_ctc']} // ⬅️ only 'remarks' is editable
